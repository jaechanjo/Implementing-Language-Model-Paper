# -*- coding: utf-8 -*-
"""1 - Sequence to Sequence Learning with Neural Networks.ipynb(NLP-Assignment1)

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1vAZNyzjooiuOYiDjJ4pmPcJRfoeEqBNg
...
#ModuleNotFoundError: No module named 'torchtext.legacy' # Maybe legacy was removed in version 0.11.0.
!pip uninstall torchtext -y
!pip install -U torchtext==0.10.0

# spaCy to tokenize our data.
...
# 소스 문장의 단어 순서를 역순으로 변경하는 것은 LSTM이 직면하는 vanishing gradient 문제를 완화시킵니다. 
# 따라서 역순으로 된 문장에서는 target 문장의 첫 번째 단어와 source 문장의 마지막 단어가 매핑되므로, 
# 더 많은 짧은-term dependencies가 도입되어 LSTM 모델을 학습시키기가 더 쉬워집니다. --> 소스인 독일어만 순서를 역순으로 
...
# 입력 텍스트 문장을 받아서, 특정 국가 언어에 따른 모듈로 토큰화하고, 그리고 시작 토큰과 끝 토큰을 설정해준다. 그리고 모두 소문자로 받기로 한다.
...
# parallel English, German and French sentences
...
# vocabulary에 특정한 워드와 인덱스 쌍으로 정해놓는 것의 의미는, 최소한 2번 이상은 등장해야, 인덱스 수가 늘어나는 것을 줄일 수 있지. (당연한 이야기)
# 그럼 1번만 등장하는 것은? <unk> (unknown) token.
# It is important to note that our vocabulary should only be built from the training  - vocabulary의 선정 기준은 학습 데이터셋 기준이다.(정확한 평가)

SRC.build_vocab(train_data, min_freq = 2)
TRG.build_vocab(train_data, min_freq = 2)

# vocabulary space, 즉 어휘 공간의 크기 - 독일어 사전과 영어 사전의 크기
...
# 특히 LSTM/GRU 의 경우, 디폴트로  tensor의 batch dim의 위치가 the second dim인데 (보통은  tensor의  first dim이  batch dim), 
# 현재 코드도 이 default를 사용하고 있기 때문에 (batch first option을 사용하지 않음), 이에 대해서도 주의를 기울려야 합니다:

BATCH_SIZE = 128


# BucketIterator은 일반적인 Iterator와 달리 소스와 타켓 문장의 padding을 최소화 시키도록 해준다.
train_iterator, valid_iterator, test_iterator = BucketIterator.splits(
    (train_data, valid_data, test_data), 
    batch_size = BATCH_SIZE, 
    device = device)

"""### bucketIterator의 장점, 그 원리

    BucketIterator는 일반적인 Iterator와 비교하여 소스와 타겟 문장의 패딩을 최소화하는 방식으로 데이터를 배치로 구성합니다. 이는 데이터 로딩 및 처리 시간을 줄여 효율적인 학습을 가능하게 합니다.

    BucketIterator가 패딩을 최소화하는 원리는 다음과 같습니다:

    Bucketing: BucketIterator는 문장들을 길이에 따라 '버킷(bucket)'이라는 그룹으로 나눕니다. 버킷은 일정한 길이 범위 내의 문장들을 담고 있습니다. 예를 들어, 길이가 5-10인 문장들은 하나의 버킷에 모아집니다.

    Sort within bucket: 각 버킷 내에서는 문장들이 길이에 따라 정렬됩니다. 이렇게 하면 비슷한 길이의 문장들이 함께 배치되어 패딩이 최소화됩니다.

    Batching: 버킷 내의 문장들은 배치 크기(batch size)에 맞추어 그룹화됩니다. 이때, 같은 배치 내의 문장들은 길이가 비슷하기 때문에 패딩이 적게 필요합니다.

    요약하면, BucketIterator는 문장 길이를 기준으로 버킷을 형성하고, 각 버킷 내에서 문장들을 길이에 따라 정렬하며 배치를 구성합니다. 이 과정을 통해 비슷한 길이의 문장들이 함께 배치되어 패딩이 최소화되고, 학습 과정에서 메모리와 계산 리소스를 절약할 수 있습니다.
...
# 손실의 최소화하는 목적으로 최적화를 어떤 식으로 할 지에 대한 기법
...
# 토큰을 배치 단위로 받을 때, 꼭 배치 사이즈 만큼 나누어 떨어지지 않는다.
# 그때는 같은 단위를 맞추기 위한 보충제, pad token이 들어가고 이때 이 토큰은 평균 손실함수 계산에 반영되지 않도록 한다.
...
def train(model, iterator, optimizer, criterion, clip):
    ...
    for i, batch in enumerate(iterator):
        ...
        # 손실 값을 계산하기 위해서, <sos> 토큰은 제거하고 계산해주도록 한다.
...
def translate(model, iterator, SRC_vocab, TRG_vocab):
    ...
    # dropout 및 batch normalization 등과 같은 학습 과정에서 사용되는 기능이 비활성화
    model.eval()

    # gradient 계산을 비활성화하여 메모리 사용량을 줄이고 속도를 높입니다. 평가 과정에서는 gradient를 필요로 하지 않으므로 이렇게 설정
    with torch.no_grad():

        for i, batch in enumerate(iterator): # 주어진 iterator를 통해 배치(batch_size = 128) 데이터를 반복합니다.
...
# translate
translate(model, test_iterator, SRC.vocab, TRG.vocab)
